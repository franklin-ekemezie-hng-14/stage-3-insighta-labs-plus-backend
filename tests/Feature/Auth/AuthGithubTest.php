...
it('should generate PKCE code_verifier', function () {
    ...
    $this->assertTrue(Session::has('oauth_code_verifier') || Cache::has('oauth_code_verifier_'.Session::getId()));
});
...
it('should store state securely (session or cache)', function () {
    ...
    $this->assertTrue(Session::has('oauth_state') || Cache::has('oauth_state_'.$state));
});
