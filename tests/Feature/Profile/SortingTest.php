...
beforeEach(function () {
    DB::table('profiles')->insert([
        ['id' => Str::uuid7(), 'name' => 'a', 'gender' => 'male', 'gender_probability' => 0.8, 'age' => 45, 'age_group' => 'adult', 'country_id' => 'US', 'country_name' => 'US', 'country_probability' => 0.9, 'created_at' => now()->subDays(3)],
        ['id' => Str::uuid7(), 'name' => 'b', 'gender' => 'female', 'gender_probability' => 0.99, 'age' => 15, 'age_group' => 'teenager', 'country_id' => 'CA', 'country_name' => 'CA', 'country_probability' => 0.8, 'created_at' => now()->subDays()],
        ['id' => Str::uuid7(), 'name' => 'c', 'gender' => 'male', 'gender_probability' => 0.6, 'age' => 30, 'age_group' => 'adult', 'country_id' => 'UK', 'country_name' => 'UK', 'country_probability' => 0.85, 'created_at' => now()->subDays(2)]
    ]);
...
it('sorts by age asc', function () {
    ...
    expect($response->json('data.0.name'))->toBe('b')
        ->and($response->json('data.1.name'))->toBe('c')
        ->and($response->json('data.2.name'))->toBe('a');
});
...
it('sorts by age desc', function () {
    ...
    expect($response->json('data.0.name'))->toBe('a')
        ->and($response->json('data.1.name'))->toBe('c')
        ->and($response->json('data.2.name'))->toBe('b');
});
...
it('sorts by gender_probability', function () {
    ...
    expect($response->json('data.0.name'))->toBe('b')
        ...
        ->and($response->json('data.2.name'))->toBe('c'); // 0.99
    // 0.8
    // 0.6
});
